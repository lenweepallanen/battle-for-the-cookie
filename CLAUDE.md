...
- `3d.html` — **le jeu complet en FPS 3D** (Jalon 3D-2 « parité », Three.js 0.170 en ES module via cdnjs, autonome, ~2100 lignes). Portage complet du 2D sur un plan x/z en mètres (monde 140 m ≈ 2200 px, ratio ×140/2200 pour toutes les distances) : foule 120 PNJ blocky, FPS Krunker-like (sprint/saut/slide, `event.code` pour AZERTY), arsenal complet (pistolet/fusil/pompe/roquette + medkits canalisés + 26 caisses hot drop + drop à la mort), zone rétractable (mur cylindrique + minimap), PV/down/respawn 30 s, les 3 bots (IA 5 priorités, purge aux dés, faillibilité calibrée, PvP, leaderboard), fouet complet (E arme, lash sans filtre, ligotage 10 s au clic maintenu interrompu par tout dégât), sang persistant (CanvasTexture au sol = 1 draw call), screenshake, témoins. Tir joueur = raycast hitscan (élimine le piège n°1) ; balles de bots = projectiles réels 3D (visent aussi en hauteur) ; purge des bots = résolution aux dés avec balles cosmétiques (leçon clé du 2D respectée). **Multijoueur (Jalon 3D-9)** : hôte autoritatif dans le navigateur via WebRTC/PeerJS (CDN, signaling cloud PeerJS, zéro serveur), lien d'invitation `?room=XXXX`, lobby 2-4 joueurs avec bots de complément (`4 - humains`), section `NET` dans 3d.html — l'hôte fait tourner TOUTE la simulation et envoie des snapshots ~15 Hz (`buildSnapshot`) ; les invités simulent leur propre mouvement (feeling FPS local), envoient position 20 Hz + événements de confiance (`hitN/hitB/hitP/take/tie/accuse/heal`), et ne reçoivent JAMAIS `isCulprit` (accusations arbitrées par l'hôte, verdict asynchrone). Avatars distants = chibi `addRemote` (hitbox → PvP +50/−100), morts/indices/zone/loot/fins diffusés en événements, invité parti = down permanent, hôte parti = fin de partie. Tests : `e2e-multi.mjs` (2 pages Playwright, connexion PeerJS réelle). v0 : pas de drop de caisses à la mort d'un invité, roquettes d'invités ignorent les avatars. **Rendu toon « cozy » (Jalon 3D-7)** : MeshToonMaterial + rampe 4 paliers doux, contours bruns en post-process (arêtes de profondeur — la scène est rendue dans un `sceneRT` dédié couleur+profondeur que le composer ne fait que LIRE : jamais de boucle de feedback depth, sinon scintillement sur vrai GPU), palette midi saturée, FXAA (plus de MSAA), village cozy (26 arbres toon à feuillage-cover, chemins de terre peints sur le canvas du sol, fleurs en croisillons, bancs/souches/fontaine/clôtures, cheminées + fenêtres), personnages CHIBI procéduraux (`buildChibi` : tête sphère, corps capsule, bras balants animés, bob vs calotte pour long/court), mains moufles sur les viewmodels, HUD papier illustré (panneaux crème, minimap ronde, 📢/❤️), feuilles ambiantes. **Difficulté (Jalon 3D-8)** : barème progressif `SCORE_GOOD_TABLE` [10,10,15,20,25] / `MALUS_TABLE` [−10,−20,−30,−40,−50] (affiché au HUD), mort = `SCORE_DEATH` −100 (joueur ET bots) + 30 s de down, down d'un rival = +50 crédité au tueur (bots compris), bots qui LOOTENT fusil/pompe (`BOT_WEAPONS`, mode 'loot', arme visible et droppée à leur mort), purge 90 %/cadence 260 ms/recherche 60 m, et RIVALITÉ inter-bots (cible = le plus proche, joueur ou bot ; leurs balles se touchent entre eux). **Rendu (Jalon 3D-5)** : import map three/addons (jsdelivr), ombres PCFSoft 2048 dont la caméra suit le joueur (`updateProjectionMatrix()` obligatoire après les bornes !), lumière heure dorée, dôme de ciel procédural + fog accordé, textures de détail procédurales teintées (briques/planches/pierre/métal, UV à densité constante), décor statique fusionné par matériau (~12 draw calls), bloom + ACES via EffectComposer (MSAA 4×), mode `?lowfx=1` (sans ombres/post — utilisé par le e2e headless, le GPU logiciel s'effondre sinon). **ADS + sniper (Jalon 3D-6)** : visée clic droit (zoom par arme, −68 % de dispersion, −45 % vitesse, sensibilité ∝ FOV), viewmodels détaillés avec organes de visée alignés (`ADS_POSE`), échelles grimpables (`addLadder`, on pousse vers la face), tour au sniper (-36,-2, 7 m, hors zone finale) avec l'UNIQUE sniper de la map (50 balles, one-shot 95, lunette overlay, quasi parfait zoomé seulement). **Gunfeel FPS (Jalon 3D-4)** : headshots ×2 (hitbox tête séparée) + hitmarkers (blanc/or/rouge + sons), recul compensable (la visée monte, le spray l'amplifie), précision liée au mouvement (bloom immobile/course/air) affichée par le crosshair dynamique, chargeurs + rechargement (R ; medkit déplacé sur F), balles de bots à 68 m/s en traits étirés avec substeps anti-tunneling, air control + slide-hop (élan conservé), viewmodel vivant (sway/bob/roll/muzzle flash/douilles), caméra (dip d'atterrissage, roll de strafe, FOV vitesse), sons par couches avec variation de pitch + bruits de pas. **Verticalité (Jalon 3D-3)** : obstacles en AABB 3D (`minY/maxY`), physique joueur avec step-up (`STEP_UP` 0.42 m : marches, parapets), hauteur de support anti-tunneling ; décor = 4 bâtiments à toit accessible par escalier extérieur, tour de guet dans la zone finale, 2 conteneurs, 12 barricades sautables (0.8 m), 8 amas de rochers — PNJ et bots restent au sol (les toits = avantage tactique du joueur), spawns/loot/patrouilles évitent les solides (`footprintBlocked`). Duplication de logique assumée : le refactor en modules partagés (§7.1) reste la prochaine tâche de fond.
